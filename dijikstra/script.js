...
blocks.addEventListener("click", (e) => {
  if (addEdge) return;
  if (cnt > 50) {
    alert("Cannot add more than 50 Vertices");
    return;
  }
  console.log(e.x, e.y);
  appendBlock(e.x, e.y);
...
const findShortestPath = (el) => {
  ...
  let source = Number(el.previousElementSibling.previousElementSibling.value);
  let destination = Number(el.previousElementSibling.value);
